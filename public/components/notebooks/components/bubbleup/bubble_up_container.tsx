...
import { EuiAccordion, EuiFlexGrid, EuiFlexGroup, EuiFlexItem, EuiMarkdownFormat, EuiPagination, EuiSpacer, EuiText, EuiCallOut, getDefaultOuiMarkdownProcessingPlugins, EuiStepsHorizontal } from '@elastic/eui';
...
export const BubbleUpContainer = ({ index, para, http, updateBubbleParagraph, updateNotebookContext }: Props) => {
  ...
  useEffect(() => {
    const loadSpecsData = async () => {
      ...
      try {
        ...
        dataService.setConfig(dataSourceId, index, timeRange.from, timeRange.to, timeField, context.PPLFilters);
  ...
  useEffect(() => {
    ...
    const loadSummary = async () => {
      ...
      try {
        ...
        IMPORTANT: When mentioning Available field names (${disSummary.slice(0, 15).map(spec => spec.field)}) in your analysis, wrap them with [FIELD:fieldname] tags. Use the exact field names from the data below. Bold the content that you feel confident about.
      ...
      <div>
        ...
        <EuiCallOut
          ...
          title={<EuiText size="s">
            <strong>Interactive Analysis:</strong> Click on the highlighted field names in the analysis above to show visualization.
          </EuiText>}
          iconType="help"
        />
      </div>
  ...
  const specsVis = !specsLoading && summary &&(
    <EuiAccordion id="accordion1" buttonContent="Show the key field visualizations" initialIsOpen={true}>
      <EuiSpacer size="m" />
  ...
  const steps: EuiContainedStepProps[] = [
    ...
    // },
    {
      ...
      children: !summaryLoading && summary ? (
        <EuiAccordion id="accordion2" buttonContent="Show the anomaly summary" initialIsOpen={true}>
          <EuiSpacer size="m" />
          <div>
            <div ref={markdownRef}>
  ...
  return (
    <>
      ...
      <EuiSpacer size="m" />
      {specsVis}
      {showModel && <BubbleUpModel title={`Anomaly Visualization -${selectedField}`} closeModal={() => { setShowModel(false) }} bubbleUpSpec={bubbleUpSpecs.filter((spec) => spec?.title?.text === selectedField)[0]} />}
    </>
  );
};