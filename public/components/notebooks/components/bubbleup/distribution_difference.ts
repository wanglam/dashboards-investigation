...
import { DataPublicPluginStart, ISearchStart, OPENSEARCH_FIELD_TYPES } from "../../../../../../../src/plugins/data/public";
...
class BubbleUpDataDistributionService {
    ...
    private async fetchIndexData(
        ...
        try {
            const response = await firstValueFrom(this.search.search({
    ...
    public async fetchComparisonData2(
        timeField: string,
        dataSourceId: string,
        index: string,
        ...
        const baselineStartTime = new Date(selectionTime.getTime() - 1000 * 600);
        const baselineData = await this.fetchIndexData(
            timeField,
            dataSourceId,
            index,
            baselineStartTime,
    ...
    public async fetchComparisonData(
        timeField: string,
        dataSourceId: string,
        index: string,
        ...
        // const baselineStartTime = new Date(baselineEndTime.getTime() - selectionDurationMs);
    ...
    public async discoverFields(
        ...
        const keywordFields = indexPattern.fields.getAll().filter(field => {
            return field.esTypes && field.esTypes.includes(OPENSEARCH_FIELD_TYPES.KEYWORD) && field.displayName;
 ...
 */
    public groupNumericKeys(
        ...
        if (allKeys.size > 30 && Array.from(allKeys).every(key => !isNaN(Number(key)))) {
            ...
            for (let i = 0; i < numGroups; i++) {
                ...
                const upperBound = i === numGroups - 1
                    ? max
                    : min + (i + 1) * groupSize;
   ...
   */
    public analyzeDifferences(
    ...
    ): Array<{ field: string; divergence: number; selectionDist: Record<string, number>; baselineDist: Record<string, number> }> {
        const results = [];
        ...
        for (const field of fieldsToAnalyze) {
            ...
            const { groupedSelectionDist, groupedBaselineDist } = this.groupNumericKeys(selectionDist, baselineDist);
    ...
    public formatComparisonSummary(
        differences: Array<{ field: string; divergence: number; selectionDist: Record<string, number>; baselineDist: Record<string, number> }>,
        ...
        return topDifferences.map(diff => {
            ...
            const allKeys = [...new Set([...Object.keys(selectionDist), ...Object.keys(baselineDist)])];
            const selectionTotal = Object.values(selectionDist).reduce((sum, count) => sum + count, 0) || 1;
            ...
            const changes = allKeys.map(value => {
                ...
                if (baselinePercentage > 0 && selectionPercentage > 0) {
                    changePercentage = ((selectionPercentage - baselinePercentage) / baselinePercentage) * 100;
                } else {
