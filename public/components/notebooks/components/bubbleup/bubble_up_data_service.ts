...
class BubbleUpDataService {
  ...
  public setConfig(dataSourceId: string, index: string, startTime: number, endTime: number, timeField: string, pplFilter?: string[]) {
  ...
  public async discoverFields(
    ...
    const keywordFields = mappings
      .filter((field) => {
        if (field.esTypes && field.name && field.type) {
          ...
          return field.esTypes.includes(OPENSEARCH_FIELD_TYPES.KEYWORD) || field.esTypes.includes(OPENSEARCH_FIELD_TYPES.BOOLEAN) || field.esTypes.includes(OPENSEARCH_FIELD_TYPES.TEXT);
        }
      })
    ...
    const patternField = this.getLogPatternField(getFlattenedObject(data.selection[0]), normalizedFields);
  ...
  ): Promise<{ field: string; divergence: number; selectionDist: Record<string, number>; baselineDist: Record<string, number>; }[]> {
    const results = [];
  ...
  private async getLogPattern() {
    ...
    const suffixPPL = `patterns ${this.logPatternField} method=brain | ` +
  ...
  private convertToRecord(data: [number, string][]): Record<string, number> {
